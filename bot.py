...
ПОДБОРКИ:
- Если просят подобрать (фильм, игра, музыка) — спроси что нравится: жанр, настроение, предпочтения, чтобы точно подобрать

ФОРМАТИРОВАНИЕ Telegram Markdown:
- Жирный: *жирный* , курсив: _курсив_
- Код для копирования ОБЯЗАТЕЛЬНО: ```\nкод\n``` — ВСЕГДА оберни копируемое (разбор 2+2, код, формулы) в ``` чтобы была кнопка копировать
- Инлайн: `код`
- Не используй ** — только * *
...
async def send_sticker_if_needed(chat_id, answer, bot):
    try:
        # ловим [STICKER:xxx] или sticker:xxx или stiker:xxx
        m = re.search(r"\[STICKER:(\w+)\]|stiker:(\w+)|sticker:(\w+)", answer, re.IGNORECASE)
        tag = None
        if m:
            tag = (m.group(1) or m.group(2) or m.group(3)).lower()
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if len(history)>20:
            history[:]=history[-20:]
        clean=answer
        clean = re.sub(r"\[STICKER:\w+\]|stiker:\w+|sticker:\w+", "", clean, flags=re.IGNORECASE).strip()
        # Фикс Telegram Markdown: ** -> * , __ -> * , и тд. Telegram не понимает **
        clean = re.sub(r"\*\*(.+?)\*\*", r"*\1*", clean)
        clean = re.sub(r"__(.+?)__", r"*\1*", clean)
        # Фикс блока кода с языком: ```python -> ```
        clean = re.sub(r"```\w+\n", "```\n", clean)
        async def send_formatted(t):
            if len(t)>4096:
                for i in range(0,len(t),4096):
                    chunk=t[i:i+4096]
                    try:
                        await update.message.reply_text(chunk, parse_mode="Markdown", reply_markup=get_main_keyboard())
                    except Exception as e:
                        logger.warning(f"markdown fail chunk {e}")
                        await update.message.reply_text(chunk, reply_markup=get_main_keyboard())
            else:
                try:
                    await update.message.reply_text(t, parse_mode="Markdown", reply_markup=get_main_keyboard())
                except Exception as e:
                    logger.warning(f"markdown fail {e} text={t[:200]}")
                    # пробуем как plain без парсинга
                    await update.message.reply_text(t, reply_markup=get_main_keyboard())
        await send_formatted(clean)
        await send_sticker_if_needed(update.effective_chat.id, answer, context.bot)
    except Exception as e:
        err=str(e)
        logger.error(err)
        if "429" in err or "quota" in err.lower():
            await update.message.reply_text("⚠️ Лимит, подожди 1 мин 🍵", reply_markup=get_main_keyboard())
