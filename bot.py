...
async def handle_message(update, context):
    ...
    try:
        ...
        # Счетчик генераций для таймера по количеству, а не времени
        gen_count = 0
        async def call_gemini_parallel():
            nonlocal gen_count
            # 3 генерации параллельно, чем раньше хорошая - тем лучше
            for attempt in range(7): # до 21 генераций
                gen_count += 3
                # если >6 генераций - показываем таймер каждую сек
                if gen_count > 6 and attempt == 2:
                    try:
                        # таймер 10с с каждой секундой
                        msg = await context.bot.send_message(chat_id=update.effective_chat.id, text="Ой 🫶 Бот перегружен, отвечу через `10с` 🍵", parse_mode="Markdown", reply_to_message_id=update.message.message_id)
                        for i in range(9, 0, -1):
                            await asyncio.sleep(1)
                            try: await context.bot.edit_message_text(chat_id=update.effective_chat.id, message_id=msg.message_id, text=f"Ой 🫶 Бот перегружен, отвечу через `{i}с` 🍵", parse_mode="Markdown")
                            except: break
                ...
                pending=set(tasks)
                while pending:
                    done, pending = await asyncio.wait(pending, return_when=asyncio.FIRST_COMPLETED, timeout=12)
                    if not done: break
                    for d in done:
                        try:
                            res=d.result()
