...
# Автообновление стикеров каждые 5 мин для всех чатов
def sticker_auto_refresh():
    import time
    while True:
        time.sleep(300)
        try: load_all_stickers()
        except: pass
threading.Thread(target=sticker_auto_refresh, daemon=True).start()
...
async def send_sticker_if_needed(chat_id, answer, bot):
    try:
        ...
        # тюлень -> ищем по эмодзи 🦭 или слову
        if tag in ["seal","тюлень","тюленя","seal_emoji"] or "тюлен" in answer.lower() or "seal" in answer.lower():
            try: load_all_stickers()
            except: pass
            seal_ids=[fid for fid,emo in DYNAMIC_EMOJI_MAP.items() if "🦭" in emo]
            # если в паке нет 🦭 - берем любой милый (angel/happy)
            if seal_ids:
                await bot.send_sticker(chat_id=chat_id,sticker=random.choice(seal_ids))
                return
            # фолбэк: если нет тюленя в паке - шлем 🦭 эмодзи + рандом милый
            try:
...
async def handle_message(update, context):
    ...
    try:
        ...
        async def call_gemini():
            models=[GEMINI_MODEL,"gemini-1.5-flash-8b","gemini-1.5-flash","gemini-flash-latest"]
        ...
        # Бесконечно пока не получит без ошибки, таймер совпадает с ответом
        gen_count = 0
        timer_msg = None
        async def timer_loop():
            ...
            while True:
                ...
                if gen_count > 6 and timer_msg is None:
                    try:
                        timer_msg = await context.bot.send_message(chat_id=update.effective_chat.id, text="Ой 🫶 Бот перегружен, отвечу через `10с` 🍵", parse_mode="Markdown", reply_to_message_id=update.message.message_id)
                    except: pass
                if timer_msg is not None:
                    try:
                        left = max(0, 10 - (gen_count - 6)//3)
                        await context.bot.edit_message_text(chat_id=update.effective_chat.id, message_id=timer_msg.message_id, text=f"Ой 🫶 Бот перегружен, отвечу через `{left}с` 🍵", parse_mode="Markdown")
                    except: pass
        async def call_gemini_parallel():
            nonlocal gen_count, timer_msg
            ttask = asyncio.create_task(timer_loop())
            try:
                while True: # бесконечно пока не без ошибки
                    gen_count += 3
                    ...
                    while pending:
                        ...
                        for d in done:
                            try:
                                res=d.result()
                                if res and getattr(res,"text",None) and str(res.text).strip():
                ...
                ttask.cancel()
                if timer_msg:
                    try: await context.bot.delete_message(chat_id=update.effective_chat.id, message_id=timer_msg.message_id)
                    except: pass
