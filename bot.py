...
async def handle_message(update, context):
    ...
    try:
        ...
        if "429" in err or "quota" in err:
            txt="Хихи 🫶 Лимит 15/мин — подожди 10с и попробуй снова 💗"
        elif "404" in err:
            txt="Ой 🥺 Модель устарела, меняю 🫶"
        elif "parts" in err:
            txt="Ой 🥺 История сломалась — очистила 🍵"
        else:
            txt=f"Ой 🥺 Ошибка: {str(e)[:200]} 🫶"
        try:
            await context.bot.send_message(chat_id=update.effective_chat.id,text=txt,reply_markup=get_main_keyboard(update))
        except: pass
