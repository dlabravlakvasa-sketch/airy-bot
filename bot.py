...
Форматирование: *жирный* _курсив_ `код` ```код``` (копировать). Не используй **.
...
async def send_sticker_if_needed(chat_id, answer, bot):
    try:
        ...
        if tag and tag in STICKERS:
            await bot.send_sticker(chat_id=chat_id,sticker=STICKERS[tag]); return
...
async def handle_message(update, context):
    ...
    try:
        ...
        err=str(e).lower()
        logger.error(str(e))
        # 404 показываем, остальное тихо ретраим и отвечаем
        if "404" in err:
        ...
        if "429" in err or "quota" in err:
            # тихо ждем и пробуем снова без уведомления
            await asyncio.sleep(3)
            try:
                # повторная попытка с другим ключом
                rotate_key()
                response=await call_gemini()
        ...
        if "parts" in err:
            ...
            return
        try:
            await context.bot.send_message(chat_id=update.effective_chat.id,text="Хихи 🫶 Чуть задумалась, попробуй еще раз 💗",reply_markup=get_main_keyboard(update))
        except: pass
