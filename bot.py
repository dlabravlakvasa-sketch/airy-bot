...
def load_all_stickers():
 ...
 # Pinterest и видео убраны по просьбе
...
async def send_countdown(chat_id, context, seconds, start_text, end_text, kb):
    try:
        ...
        # редактируем раз в 2 сек чтобы не ловить Flood control
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(global_requests) > 40:
        await asyncio.sleep(2)
        # тихо пропускаем без уведомления
    ...
    try:
        ...
        async def call_gemini():
            models_to_try=[GEMINI_MODEL,"gemini-1.5-flash","gemini-1.5-flash-001","gemini-flash-latest","gemini-2.0-flash"]
            safety = [
            ...
            for ki in range(len(GEMINI_KEYS)):
                for m in dict.fromkeys(models_to_try):
                    try:
                        return await asyncio.to_thread(lambda m=m: client.models.generate_content(model=m, contents=contents, config=cfg))
                    except Exception as e:
                        es=str(e)
                        ...
                        if "429" in es or "quota" in es.lower() or "RESOURCE_EXHAUSTED" in es:
                            last=e; continue
                        raise
                # все модели для этого ключа уперлись - пробуем следующий ключ
                if last and "429" in str(last) and ki < len(GEMINI_KEYS)-1:
                    rotate_key()
                    continue
            raise last
        ...
        if "429" in err or "quota" in err.lower() or "RESOURCE_EXHAUSTED" in err:
            # Показываем таймер и потом автоматом отвечаем
            await send_countdown(update.effective_chat.id, context, 10, "Хихи 🫶 Бот перегружен 💗\nПодожди чуток... 🍵", "Готово ✨ Пробую снова... 🫶", kb)
            try:
                # повторная попытка после паузы - пробуем снова всех
                for name, func in [("ChatGPT", call_openai), ("Grok", call_xai), ("Gemini", call_gemini)]:
                    try:
                        if name=="ChatGPT" and not OPENAI_KEYS: continue
                        if name=="Grok" and not XAI_KEYS: continue
                        ...
                        if answer:
                            ...
                            clean2 = re.sub(r"\[?\s*(?:sticker|stiker)\s*:\s*\w+\s*\]?", "", answer, flags=re.IGNORECASE).strip().replace("😂","").replace("🤣","")
                            def md2(t):
                            ...
                            is_group = update.effective_chat.type in ["group", "supergroup"]
                            if is_group:
                                await context.bot.send_message(chat_id=update.effective_chat.id, text=html2, parse_mode="HTML", reply_to_message_id=update.message.message_id, reply_markup=kb)
                            else:
                                await update.message.reply_text(html2, parse_mode="HTML", reply_markup=kb)
                            await send_sticker_if_needed(update.effective_chat.id, answer, context.bot)
                            return
                    except: continue
            except: pass
            await context.bot.send_message(chat_id=update.effective_chat.id, text="Ой 🥺 Все ИИ заняты, попробуй через 10с 🫶", parse_mode="Markdown", reply_markup=kb)
            return
        text = airy_error_text(err)
        if "INVALID_ARGUMENT" in err and "parts" in err:
            chat_histories[user_id] = []
