...
SYSTEM_PROMPT = f"""Тебя зовут Airy 💗. Ты милый мальчик-ассистент в Telegram, тебе 18 лет. ТВОЙ СТИЛЬ ФИКСИРОВАН И НЕ МЕНЯЕТСЯ НИКОГДА - независимо от модели (ChatGPT, Grok, Gemini ты всегда одинаковый).
...
async def handle_sticker(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        # 10% шанс красиво + распознать, 90% просто мило
        if random.random() > 0.1:
            # 90% - не распознает или просто кинет стикер в ответ
            if random.random() < 0.5:
                await context.bot.send_sticker(chat_id=update.effective_chat.id, sticker=random.choice(list(STICKERS.values())))
            else:
                await context.bot.send_message(chat_id=update.effective_chat.id, text=random.choice(["Милый стикер 🫶", "Хихи 💗", "Котик 🦭✨", "Ня 🩷"]))
            return
        sticker = update.message.sticker
        fid = sticker.file_id
        emoji = sticker.emoji or ""
        desc = STICKER_EMOJI_MAP.get(fid, f"стикер {emoji}")
        prompt = f"[Пользователь отправил стикер: {desc} {emoji}] Отреагируй КРАСИВО и КРАТКО по теме стикера, 1 строка, эстетично 💗"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        answer=None
        # Приоритет: ChatGPT -> Grok (xAI) -> Gemini
        try:
            if OPENAI_KEYS:
            ...
            logger.warning(f"ChatGPT fail {e}, try Grok")
            try:
                if XAI_KEYS:
                ...
                logger.warning(f"Grok fail {e2}, fallback to Gemini")
