...
global_requests = [] # для лимита в минуту
last_error_time = {}
...
def search_pinterest(query, limit=3):
    try:
        ...
        # 1. Пробуем Pinterest (с экранированными URL)
        try:
            url=f"https://www.pinterest.com/search/pins/?q={requests.utils.quote(query)}"
            r=requests.get(url, headers=headers, timeout=12)
            # ловим и обычные и экранированные https:\/\/i.pinimg.com
            ...
            for u in urls:
                ...
                if "i.pinimg.com" in u and u not in uniq:
                    uniq.append(u)
                if len(uniq)>=limit*2:
                    break
            if uniq:
        ...
        # 2. Фолбэк на Unsplash/Bing если Pinterest пусто (Render часто блокирует Pinterest)
        try:
            # Unsplash
            r2=requests.get(f"https://unsplash.com/s/photos/{requests.utils.quote(query)}", headers=headers, timeout=10)
            urls2=re.findall(r"https://images\.unsplash\.com[^\"]+", r2.text)
            if urls2:
                uniq2=list(dict.fromkeys(urls2))[:limit]
                return uniq2
        except: pass
        # 3. Еще фолбэк - picsum
        return [f"https://picsum.photos/seed/{query}{i}/800/600" for i in range(limit)]
    except Exception as e:
...
async def download_and_send_video(url, chat_id, context):
    try:
        await context.bot.send_message(chat_id=chat_id, text="Скачиваю видео без водяного знака... 🫶✨", reply_to_message_id=None)
        # Нормализуем ссылку (убираем параметры)
        url = url.strip().split("?")[0].split("&")[0] if "tiktok" not in url else url.strip()
        # 1. TikTok через tikwm (самый надежный)
        if "tiktok.com" in url or "vm.tiktok" in url or "vt.tiktok" in url:
            for api in [f"https://www.tikwm.com/api/?url={url}", f"https://api.tikmate.app/api/lookup?url={url}"]:
                try:
                    r = requests.get(api, timeout=12, headers={"User-Agent":"Mozilla/5.0"}).json()
                    play = r.get("data",{}).get("play") or r.get("video",{}).get("url") or r.get("play")
                    if play:
                        await context.bot.send_video(chat_id=chat_id, video=play, caption="Вот видео без водяного 💗🫶")
                        return True
                except: continue
        # 2. yt-dlp для всех (Instagram, Pinterest, TikTok)
        try:
            import yt_dlp
            opts = {"quiet": True, "no_warnings": True, "format": "mp4/best/best", "noplaylist": True, "nocheckcertificate": True}
            with yt_dlp.YoutubeDL(opts) as ydl:
                info = ydl.extract_info(url, download=False)
                # ищем прямой URL
                vurl = info.get("url")
                if not vurl and info.get("entries"):
                    vurl = info["entries"][0].get("url")
                if not vurl and info.get("requested_formats"):
                    vurl = info["requested_formats"][-1].get("url")
                if vurl:
                    await context.bot.send_video(chat_id=chat_id, video=vurl, caption="Вот видео 💗🫶")
                    return True
                # если есть прямой mp4 в info
                if info.get("ext") == "mp4" and vurl:
                    await context.bot.send_video(chat_id=chat_id, video=vurl, caption="Вот видео 💗")
                    return True
        except Exception as e:
            logger.warning(f"yt-dlp fail {url}: {e}")
        # 3. Инстаграм фолбэк через ddinstagram
        if "instagram.com" in url:
            try:
                # меняем instagram.com на ddinstagram.com для прямого видео
                dd = url.replace("instagram.com", "ddinstagram.com").replace("www.", "")
                await context.bot.send_video(chat_id=chat_id, video=dd, caption="Попробуй скачать по ссылке 💗")
                return True
            except: pass
        await context.bot.send_message(chat_id=chat_id, text="Ссылка не подходит 🥺 Проверь что это TikTok / Instagram / Pinterest видео 🫶\nПример: `https://vm.tiktok.com/XXXX`", parse_mode="Markdown")
        return False
    except Exception as e:
        logger.error(f"video download {e}")
        await context.bot.send_message(chat_id=chat_id, text="Ошибка скачивания 🥺 Попробуй другую ссылку 🫶")
        return False
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    import time
    now = time.time()
    # Глобальный лимит 40 запросов/мин чтобы не триггерить 429
    global_requests[:] = [t for t in global_requests if now - t < 60]
    if len(global_requests) > 40:
        await send_countdown(update.effective_chat.id, context, 5, "Ой 🥺 Я чуть устала от много запросов 🫶", "Готово ✨ Давай снова 💗", get_main_keyboard(update))
        return
    ...
    try:
        ...
        # антиспам ошибок - не чаще 1 в 20 сек на чат
        now2 = time.time()
        chat_id = update.effective_chat.id
        if chat_id in last_error_time and now2 - last_error_time[chat_id] < 20 and "429" in err:
            return
        last_error_time[chat_id] = now2
        if "429" in err or "quota" in err.lower() or "RESOURCE_EXHAUSTED" in err:
            await send_countdown(chat_id, context, 10, text, "Готово ✨ Лимит прошел, можешь продолжить 🫶💗", kb)
